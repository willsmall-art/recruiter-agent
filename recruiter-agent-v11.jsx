...
// Section headings Claude will use — kept explicit so parsing is reliable
...
export default function RecruiterAgent() {
  const [ashbyKey, setAshbyKey] = useState("2dc114535d967d6ff02172a637ec5e563e70720bbe93f2503d6968a1ba1c5112");
  ...
  /* ── toggle a button before generation ── */
  ...
  const ashbyPost = async (endpoint, body) => {
    const res = await fetch(`${ASHBY_API}${endpoint}`, {
      method: "POST",
      headers: {
...
[{"id":"","name":"","email":"","currentTitle":"","currentCompany":"","previousRole":"","stageReached":"","appliedAt":"","archiveReason":"","matchScore":85,"matchReason":"2-3 sentences","suggestedOutreach":"Warm re-engagement message under 80 words, [CANDIDATE NAME] placeholder, no 'I hope this finds you well', soft CTA at end."}]`;
  ...
  /* ── Build the text-output prompt using explicit headings ── */
...
TOV: warm, direct, specific, no corporate fluff. Second person in "about you" section.`;
  ...
  /* ── Parse sections by explicit headings ── */
  const parseResult = (text, outputIds) => {
    ...
    outputIds.forEach((id) => {
      ...
      // find the next heading from any possible section
  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", minHeight: "100vh", background: "#f0eeeb", color: "#1a1a2e", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "28px 20px" }}>
        ...
        {/* Heading — only before first generation */}
        ...
        {/* ── Main card — always visible when not loading ── */}
        {!loading && (
          <div style={{ background: "#fff", borderRadius: 16, padding: "20px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", marginBottom: 24 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
              {OUTPUTS.map((o) => {
                const isSilverLocked = false;
                const isOn    = selected[o.id];
                const isReady = hasGenerated && (o.id === "silver" ? Array.isArray(generated.silver) : !!generated[o.id]);
                ...
                else if (isOn && !isSilverLocked) { border = "1.5px solid #d0cfe8"; bg = "#f0effe"; color = "#5b5bd6"; }

                return (
                  <button
                    key={o.id}
                    onClick={() => {
                      if (isSilverLocked) return;
                      if (hasGenerated && isReady) {
                        // navigate to section
                        setActiveTab(o.id);
                      } else if (hasGenerated && !isReady) {
                        // section wasn't generated (was toggled off) — do nothing
                      } else {
                        // pre-generation: toggle inclusion
                        toggleOutput(o.id);
                      }
                    }}
                    title={isSilverLocked ? "Connect Ashby to enable" : hasGenerated && !isReady ? "Not generated — regenerate with this toggled on" : ""}
                    style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", borderRadius: 20, border, background: bg, color, fontSize: 13, fontWeight: 500, cursor: isSilverLocked ? "not-allowed" : "pointer", opacity: isSilverLocked ? 0.45 : hasGenerated && !isReady ? 0.4 : 1, transition: "all 0.15s" }}
                  >
                    <span style={{ fontSize: 14 }}>{o.emoji}</span>
                    {o.label}
                    {isReady && !isActive && (
                      <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#5b5bd6", display: "inline-block", marginLeft: 2 }} />
                    )}
                  </button>
                );
              })}
            ...
            <button
              ...
              style={{
                ...
                background: notes.trim() && OUTPUTS.filter((o) => o.id !== "silver" && selected[o.id]).length > 0
                  ? "linear-gradient(135deg, #f05a28 0%, #e8440a 100%)" : "#e8e8e8",
                color: notes.trim() && OUTPUTS.filter((o) => o.id !== "silver" && selected[o.id]).length > 0 ? "#fff" : "#bbb",
                border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700,
                cursor: notes.trim() ? "pointer" : "not-allowed", transition: "all 0.15s", letterSpacing: "0.01em",
              }}
            >
